...
export default function Car() {
  ...
  const toggleHistory = async (h: any) => {
    ...
    if (!historyDetails[h.id]) {
      try {
        const res = await fetch(`http://localhost:3001/api/cars/history/${h.id}/${h.rc_id}`);
        const data = await res.json();
        if (data?.ok) {
          setHistoryDetails(prev => ({ ...prev, [h.id]: data }));
        }
      } catch (e) {
  ...
  const fetchHistory = async () => {
    ...
    try {
      const url = new URL("http://localhost:3001/api/cars/history");
      if (qp.plate) url.searchParams.set("plate", qp.plate);
      if (qp.serial_number) url.searchParams.set("serial_number", qp.serial_number);
  ...
  return (
    <div className={styles.container}>
      ...
      <section className={styles.listArea}>
        {history.length === 0 ? (
          <div style={{ textAlign: "center", color: "#9ca3af", paddingTop: "80px" }}>No jacket history found for this vehicle.</div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
            <h3 style={{ fontSize: "16px", fontWeight: "bold", color: "#f97316" }}>Jacket History</h3>
            {history.map((h: any, idx) => {
              const details = historyDetails[h.id];
              return (
              <div key={idx} style={{ 
                ...
                >
                  <div>
                    <div style={{ fontWeight: "600", color: "#111827", marginBottom: "4px" }}>
                      Jacket Inspection {history.length - idx}
                    </div>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: "12px", fontWeight: "500", color: "#374151" }}>
          ...
          <div className={styles.modal}>
            ...
            <div className={styles.modalBody}>
              <div className={styles.sectionBody}>
                <div style={{ marginBottom: "20px" }}>
                  <h3 style={{ fontSize: "16px", fontWeight: "bold", marginBottom: "8px", color: "#f97316" }}>Receiving Checklist Items</h3>
                  {Object.keys(checks).filter(k => checks[k]).length > 0 ? (
                    <ul style={{ listStyleType: "disc", paddingLeft: "20px", display: "flex", flexDirection: "column", gap: "4px" }}>
                      {Object.keys(checks).filter(k => checks[k]).map(it => (
                        <li key={it} style={{ fontSize: "14px", fontWeight: "600", color: "#374151" }}>{it}</li>
                      ))}
                    </ul>
                  ) : (
                    <p style={{ fontSize: "14px", color: "#9ca3af" }}>No items selected</p>
                  )}
                </div>

                <div>
                  <h3 style={{ fontSize: "16px", fontWeight: "bold", marginBottom: "8px", color: "#f97316" }}>Detail Inspection Items</h3>
                  {Object.keys(checks2).filter(k => checks2[k]).length > 0 ? (
                    <ul style={{ listStyleType: "disc", paddingLeft: "20px", display: "flex", flexDirection: "column", gap: "4px" }}>
                      {Object.keys(checks2).filter(k => checks2[k]).map(it => (
                        <li key={it} style={{ fontSize: "14px", fontWeight: "600", color: "#374151" }}>{it}</li>
                      ))}
                    </ul>
                  ) : (
            ...
            <div className={styles.modalFooter}>
              ...
              <button
                ...
                onClick={async () => {
                  try {
                    const selectedChecks = Object.keys(checks).filter(k => checks[k]);
                    const selectedChecks2 = Object.keys(checks2).filter(k => checks2[k]);

                    const res = await fetch("http://localhost:3001/api/checklists/complete", {
                      method: "POST",
                    ...
                    setChecks(Object.fromEntries(Object.keys(checks).map(k => [k, false])));
                    setChecks2(Object.fromEntries(Object.keys(checks2).map(k => [k, false])));
      ...
      {expandedHistory !== null && (
        <div
          ...
          <div
            ...
            <div className={styles.modalBody}>
              {!historyDetails[expandedHistory] ? (
                <div style={{ color: "#6b7280", padding: "40px", textAlign: "center", fontSize: "15px" }}>Loading details...</div>
              ) : (
                <div style={{ display: "flex", flexDirection: "column", gap: "24px", padding: "10px 0" }}>
                  <div>
                    <h3 style={{ fontSize: "16px", fontWeight: "bold", color: "#f97316", marginBottom: "12px" }}>Receiving Checklist Items</h3>
                    {historyDetails[expandedHistory].rc_list?.length > 0 ? (
                      <ul style={{ paddingLeft: "20px", listStyleType: "disc", display: "flex", flexDirection: "column", gap: "6px" }}>
                        {historyDetails[expandedHistory].rc_list.map((it: string, i: number) => <li key={i} style={{ fontSize: "14px", fontWeight: "600", color: "#374151" }}>{it}</li>)}
                      </ul>
                    ) : <div style={{ fontSize: "14px", color: "#9ca3af" }}>None selected</div>}
                  </div>
                  <div>
                    <h3 style={{ fontSize: "16px", fontWeight: "bold", color: "#f97316", marginBottom: "12px" }}>Detail Inspection Items</h3>
                    {historyDetails[expandedHistory].di_list?.length > 0 ? (
                      <ul style={{ paddingLeft: "20px", listStyleType: "disc", display: "flex", flexDirection: "column", gap: "6px" }}>
                        {historyDetails[expandedHistory].di_list.map((it: string, i: number) => <li key={i} style={{ fontSize: "14px", fontWeight: "600", color: "#374151" }}>{it}</li>)}
                      </ul>
                    ) : <div style={{ fontSize: "14px", color: "#9ca3af" }}>None selected</div>}
                  </div>
                </div>
